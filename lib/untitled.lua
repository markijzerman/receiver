...
mod.hook.register("script_pre_init","my init hacks",function()
  ...
  local getStreams = io.popen([[wget -q -O - https://broadcast.norns.online --no-check-certificate | sed -n 's/.*href="\([^"]*\).*/\1/p' | grep '.mp3$']])
  local result = getStreams:read("*a")
  ...
  for k,v in pairs(streamURL) do
    ...
    if isArchive == true then
      local month = v:sub(15, 16)
      local day = v:sub(17, 18)
      ndate = month.. "-" ..day
      local name = ndate.. " " ..v:sub(24, -5)
      table.insert(archivedStreamsName, name)
      local url = 'https://broadcast.norns.online' ..v
      table.insert(archivedStreamsURL, url)
    else
      -- now make list of the live streams check this though, not tested
      local name = v:sub(2, -5)
      table.insert(liveStreamsName, name)
      local url = 'https://broadcast.norns.online' ..v.. " (live)"
      ...
      if debounce>0 then
        ...
        if debounce==0 then
          ...
          if params:get("STATIONS")==1 then
            ...
            local url=stations[params:get("STATIONS")]
